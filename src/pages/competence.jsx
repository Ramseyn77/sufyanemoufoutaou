        ...
        <div className={`rounded-2xl border p-6 ${style.bg} ${style.border}`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {current.techs.map((tech) => (
              <div key={tech.nom} className="flex flex-col gap-2 bg-white dark:bg-[#0D1B2A] rounded-xl p-3 hover:scale-[1.03] transition">
                ...
                <div className="flex flex-wrap gap-2">
                  {cat.techs.map((tech) => (
                    <div key={tech.nom} className="flex items-center gap-1 bg-white/60 dark:bg-black/20 rounded-lg px-2 py-1">
                      ...
                      <span className="text-[11px] text-[#0D1B2A] dark:text-[#E0E1DD] font-medium">{tech.nom}</span>
                    </div>
                  ))}
                </div>
              </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {data.certifs.map((certif, i) => (
              <div key={i} className="flex flex-col gap-2 bg-[#f3f4f6] dark:bg-[#1B263B] rounded-2xl p-4 border border-amber-400/20">
                ...
                <h3 className="text-sm font-bold text-[#0D1B2A] dark:text-[#E0E1DD] leading-snug">{certif.titre}</h3>
                {certif.description && (
